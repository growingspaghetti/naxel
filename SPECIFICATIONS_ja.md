...
| `del <コレクション> <名前>` | エントリの全バージョンをソフト削除する（リポジトリ内のファイルをドットプレフィックスに改名する；読み取りコマンドからは無視される） |
...
| `removeitems <コレクション> <名前> [-] [--json]` | `searchitems` と同じクエリエディタを開き、保存・終了するとマッチしたレコードを削除してpushし、「removed N items」を表示する。クエリが空（条件なし）の場合はマッチなしとして処理し「no matching … — nothing removed」を表示する。`--json` / `-` は `searchitems` と同様 |
...
| `nx` | ナビゲータGUIウィンドウを開く。ドロップダウンに全コレクションが表示され、選択するとその下の一覧にエントリ名が表示される。`cat`・`get`・`diff` の2行ボタンで選択エントリにコマンドを実行する（上段は `--jtable` あり、下段は `--jtable` なし）。実行したコマンド文字列はターミナルに表示され、readlineヒストリにも追加される |
| `exit` | ツールを終了する |
...
`del <コレクション> <名前>` は**ソフト削除**を行います。対象エントリの全バージョンのファイルが、リポジトリディレクトリ内でファイル名の先頭にドット（`.`）を付けた名前に改名されます。
...
- 改名されたファイルはリポジトリに残り、物理的には削除されません。
- ドットプレフィックスのファイルは**すべての読み取りコマンドから無視されます**：`ls`、`cat`、`get`、`clear`、`len`、`diff`、`push`（参照検証）、`export`、`fullcopy --json`、`partialcopy --json` はすべてドットで始まるファイル名をスキップします。
- `del` はリポジトリでの改名直後に**ローカルキャッシュ**の対応ファイルも削除するため、再起動なしに現在のセッションへ削除が反映されます。
- 次回 `sync_cache` 実行時（起動時・`cd`・`export`）に、リポジトリ側に対応する非ドットファイルが存在しないキャッシュファイルが自動的に削除され、他ユーザーへも削除が伝播します。
...
- `removeitems`：マッチしたレコードを削除してpushし、「removed N items」を表示します。クエリが空（条件なし）の場合はマッチなしとして処理し「no matching … — nothing removed」を表示します。
...
空クエリ（条件なし）は `searchitems`（`[]` を返す）・`removeitems`（「no matching … — nothing removed」を表示）いずれも0件マッチとして処理されます。
...
- 空オブジェクト `{}` は0件マッチとして処理されます（バッククォート構文の空クエリと同じ動作）。
