...
| `cat <collection> <name> --version=N`   | Print a specific version to stdout; N is a bare integer or zero-padded (e.g. `3` or `0003`) |
| `cat <collection> <name> --jtable`      | Save to `downloads/{collection}/`, open read-only JTable window |
| `cat <collection> <name> --version=N --jtable` | Save a specific version to `downloads/{collection}/`, open read-only JTable window |
...
| `cat <collection> <name> --version=N --json` | Print a specific version as JSON to stdout. Mutually exclusive with `--jtable`. |
...
| `appenditems <collection> <name> [-] [--json]` | Open a text editor pre-filled with an empty record template; on save-and-close, parse the new content and append it to the existing entry, then push. Main collection default input: 👉👈 text; with `--json`: JSON array of section objects. Reference collection default: comma-separated values; with `--json`: JSON array of strings. If the existing entry is in its initial (all-blank) state the blank template is replaced, not extended. `-` reads content from stdin instead of opening an editor (for `-c` pipelines). |
| `searchitems <collection> <name> [-] [--json]` | Open a filter query editor; on submit, print matching sections/values as a JSON array to stdout. Results are always JSON regardless of `--json`. Default query syntax: `` `column`='value' `` / `` `column` like 'pattern%' `` combined with `and` / `or`; empty query matches nothing (returns `[]`). With `--json`: query is a JSON object `{"col": "val"}` — all pairs ANDed, values containing `%` or `_` use LIKE matching; empty object `{}` also matches nothing. `-` reads the query from stdin. |
| `removeitems <collection> <name> [-] [--json]` | Same query editor as `searchitems`; on submit, remove matching records, push, and print "removed N items". Empty query matches nothing (prints "no matching … — nothing removed"). Same `--json` and `-` semantics. |
