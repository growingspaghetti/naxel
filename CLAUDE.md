...
| `searchitems <collection> <name> [-] [--json]` | Open a filter query editor; on submit, print matching sections/values as a JSON array to stdout. Results are always JSON regardless of `--json`. Default query syntax: `` `column`='value' `` / `` `column` like 'pattern%' `` combined with `and` / `or`; empty query matches everything. With `--json`: query is a JSON object `{"col": "val"}` — all pairs ANDed, values containing `%` or `_` use LIKE matching. `-` reads the query from stdin. |
| `removeitems <collection> <name> [-] [--json]` | Same query editor as `searchitems`; on submit, remove matching records, push, and print "removed N items". Empty query (no conditions written) is rejected to prevent accidental wipeout. Same `--json` and `-` semantics. |
