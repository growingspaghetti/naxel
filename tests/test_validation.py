...
def sys_doc(*rows, props=()):
    ...
    for machine, time, notes in rows:
        section = [SEP, M, machine, T, time, N, notes]
...
class TestValidateSystem:
    def test_single_section_valid(self):
        ok, _ = _validate_system(
            ...
            ISC_PROPS, prop_validation_types=ISC_VALIDATION,
        )
        assert ok
    ...
    def test_multiple_sections_valid(self):
        ...
        ok, _ = _validate_system(content, ISC_PROPS, prop_validation_types=ISC_VALIDATION)
        assert ok

    def test_multiline_notes_valid(self):
        content = "\n".join([SEP, M, "m1", T, "12:00", N, "line1", "line2", I, "id1", S, "s1", C, "cont1"]) + "\n"
        ok, _ = _validate_system(content, ISC_PROPS, prop_validation_types=ISC_VALIDATION)
        assert ok
    ...
    def test_wrong_label_rejected(self):
        content = "\n".join([SEP, "👉wrong👈", "m1", T, "12:00", N, "notes"]) + "\n"
        ok, _ = _validate_system(content)
        assert not ok
...
class TestParseSystemSections:
    def test_single_section(self):
        ...
        assert sections[0] == {"machine": "m1", "time": "12:00", "notes": "notes", "id": "id1", "schedule": "s1", "contact": "cont1"}
    ...
    def test_empty_template_has_blank_fields(self):
        content = "\n".join([SEP, M, "", T, "", N, "", I, "", S, "", C, ""]) + "\n"
        sections = _parse_system_sections(content, ISC_PROPS)
        assert sections[0] == {"machine": "", "time": "", "notes": "", "id": "", "schedule": "", "contact": ""}
...
class TestAdditionalPropsValidation:
    def test_valid_with_props(self):
        ...
        ok, _ = _validate_system(doc, ISC_PROPS + PROPS, prop_validation_types=ISC_VALIDATION)
        assert ok
    ...
    def test_missing_prop_label_rejected(self):
        ...
        ok, msg = _validate_system(doc, ISC_PROPS + PROPS, prop_validation_types=ISC_VALIDATION)
        assert not ok
        assert "p1" in msg
    ...
    def test_notes_terminated_by_prop_label(self):
        ...
        sections = _parse_system_sections(doc, ISC_PROPS + PROPS)
        assert sections[0]["notes"] == "line1"
        assert sections[0]["p1"] == "v1"

    def test_multiline_notes_terminated_before_props(self):
        content = "\n".join([SEP, M, "m1", T, "12:00", N, "line1", "line2",
                              I, "id1", S, "s1", C, "cont1", "👉p1👈", "val", "👉p2👈", ""]) + "\n"
        sections = _parse_system_sections(content, ISC_PROPS + PROPS)
    ...
    def test_parse_notes_not_contaminated_by_unknown_props(self):
        ...
        content = "\n".join([SEP, M, "m1", T, "12:00", N, "real notes",
                              "👉p3👈", "val"]) + "\n"
    ...
    def test_not_empty_type_rejects_empty_value(self):
        ...
        ok, msg = _validate_system(doc, ISC_PROPS + PROPS,
                                   prop_validation_types={**ISC_VALIDATION, "p1": "NOT_EMPTY"})
        assert not ok
        assert "p1" in msg
    ...
    def test_not_empty_type_accepts_non_empty_value(self):
        ...
        ok, _ = _validate_system(doc, ISC_PROPS + PROPS,
                                 prop_validation_types={**ISC_VALIDATION, "p1": "NOT_EMPTY"})
        assert ok
    ...
    def test_none_type_accepts_empty_value(self):
        doc = sys_doc(("m1", "12:00", "notes"),
                      props=[("id", "id1"), ("schedule", "s1"), ("contact", "cont1"), ("p1", ""), ("p2", "")])
        ok, _ = _validate_system(doc, ISC_PROPS + PROPS,
                                 prop_validation_types={**ISC_VALIDATION, "p1": "NONE", "p2": "NONE"})
        assert ok
    ...
    def test_hh_mm_type_accepts_valid_time(self):
        ...
        ok, _ = _validate_system(doc, ISC_PROPS + PROPS,
                                 prop_validation_types={**ISC_VALIDATION, "p2": "HH:MM"})
        assert ok
    ...
    def test_re_type_accepts_matching_value(self):
        ...
        ok, _ = _validate_system(doc, ISC_PROPS, prop_validation_types={"id": "RE:[^#]+"})
        assert ok

    def test_re_type_rejects_non_matching_value(self):
        doc = sys_doc(("m1", "12:00", "notes"),
                      props=[("id", "#bad"), ("schedule", "s1"), ("contact", "cont1")])
        ok, msg = _validate_system(doc, ISC_PROPS, prop_validation_types={"id": "RE:[^#]+"})
        assert not ok
        assert "#bad" in msg
...
class TestTextToSystemJson:
    def test_basic_section(self):
        content = sys_doc(("m1", "12:00", "notes"),
                          props=[("id", "id1"), ("schedule", "s1"), ("contact", "cont1")])
        data = json.loads(_text_to_system_json(content, ISC_PROPS))
        assert data == [{"machine": "m1", "time": "12:00", "notes": "notes", "id": "id1", "schedule": "s1", "contact": "cont1"}]
    ...
    def test_additional_props_included(self):
        content = sys_doc(("m1", "12:00", "n"),
                          props=[("id", "id1"), ("schedule", "s1"), ("contact", "cont1"), ("p1", "v1"), ("p2", "")])
        data = json.loads(_text_to_system_json(content, ISC_PROPS + ("p1", "p2")))
        assert data[0]["p1"] == "v1"
        assert data[0]["p2"] == ""
...
class TestSystemSectionsToText:
    def test_basic_conversion(self):
        sections = [{"machine": "m1", "time": "12:00", "notes": "notes", "id": "id1", "schedule": "s1", "contact": "cont1"}]
        text = _system_sections_to_text(sections, ISC_PROPS)
        assert SEP in text
        assert "m1" in text
        assert "id1" in text

    def test_multiline_notes_expanded(self):
        sections = [{"machine": "m1", "time": "12:00", "notes": "line1\nline2", "id": "id1", "schedule": "s1", "contact": "cont1"}]
        text = _system_sections_to_text(sections, ISC_PROPS)
    ...
    def test_missing_additional_prop_appended_empty(self):
        sections = [{"machine": "m1", "time": "12:00", "notes": "n"}]
